...
import { Button as WPButton, Popover, SelectControl, TextareaControl, ToolbarGroup, ToolbarButton, Dropdown } from '@wordpress/components';
	...
	return ( key, placeholder, options = {} ) =>
		( { tagName, className } ) => (
			<RichText
				...
				allowedFormats={ options.allowedFormats ?? [ 'core/bold', 'core/italic', 'core/link' ] }
 ...
 */
export function LinkButton( { value, onChange, style = 'primary', arrow = true, placeholder = __( 'Button label', 'floe' ), className = '' } ) {
	const [ isOpen, setOpen ] = useState( false );
	const anchor = useRef();
	const current = value || {};

	return (
		<span ref={ anchor } className={ `floe-link-button ${ current.url ? '' : 'is-unlinked' } ${ className }`.trim() }>
			<Button
				style={ style }
				arrow={ arrow }
				label={
			...
			<WPButton
				...
				icon={ linkIcon }
				size="small"
				label={ current.url ? __( 'Edit link', 'floe' ) : __( 'Add link', 'floe' ) }
				onClick={ () => setOpen( ! isOpen ) }
			/>
			{ isOpen && (
				<Popover anchor={ anchor.current } placement="bottom-start" onClose={ () => setOpen( false ) } focusOnMount="firstElement">
					<LinkControl
						value={ { url: current.url || '', opensInNewTab: !! current.newTab } }
						onChange={ ( next ) => onChange( { ...current, url: next.url || '', newTab: !! next.opensInNewTab } ) }
						onRemove={ () => {
							onChange( { ...current, url: '' } );
							setOpen( false );
						} }
						settings={ [ { id: 'opensInNewTab', title: __( 'Open in new tab', 'floe' ) } ] }
					/>
				</Popover>
			) }
		</span>
	...
	return useSelect(
		( select ) => {
			...
			return {
				...
				url: isVideo ? media.source_url : sizes.laptop?.source_url || sizes.large?.source_url || media.source_url,
				poster: poster?.media_details?.sizes?.laptop?.source_url || poster?.source_url,
...
export function MediaSlot( { value, onChange, ratio = '', radius = 'lg', cover = false, allowVideo = true, label = __( 'Image or looping MP4', 'floe' ), placeholder = false, className = '' } ) {
	...
	const select = ( media ) => {
		if ( media?.type === 'video' && media.mime !== 'video/mp4' && media.mime_type !== 'video/mp4' ) {
			return;
		}
		onChange( { id: media?.id, posterId: value?.posterId, alt: value?.alt } );
	};

	if ( ! value?.id ) {
		return (
			<div className={ `floe-media-slot is-empty ${ className }`.trim() } style={ ratio ? { aspectRatio: ratio } : undefined }>
				<MediaPlaceholder
					icon={ imageIcon }
					labels={ { title: label, instructions: allowVideo ? __( 'Choose an image, or an MP4 that will loop silently.', 'floe' ) : __( 'Choose an image.', 'floe' ) } }
				...
				<div className="floe-media-slot__tools">
					<ToolbarGroup>
						<MediaUpload
							...
							value={ value.id }
							onSelect={ select }
							render={ ( { open } ) => <ToolbarButton onClick={ open }>{ __( 'Replace', 'floe' ) }</ToolbarButton> }
						/>
						{ resolved.type === 'video' && (
							<MediaUpload
								allowedTypes={ [ 'image' ] }
								value={ value.posterId }
								onSelect={ ( poster ) => onChange( { ...value, posterId: poster?.id } ) }
								render={ ( { open } ) => <ToolbarButton onClick={ open }>{ value.posterId ? __( 'Poster', 'floe' ) : __( 'Add poster', 'floe' ) }</ToolbarButton> }
							/>
						) }
						<Dropdown
							...
							renderContent={ () => (
								<div className="floe-media-slot__alt">
									<TextareaControl
										...
										label={ __( 'Alternative text for this slot', 'floe' ) }
										help={ __( 'Leave empty to use the alt text from the media library.', 'floe' ) }
						...
						/>
						<ToolbarButton onClick={ () => onChange( {} ) }>{ __( 'Remove', 'floe' ) }</ToolbarButton>
					</ToolbarGroup>
				</div>
			</MediaUploadCheck>
		</div>
...
export function EditableSectionHeader( {
	...
	return (
		<SectionHeader
			...
			eyebrow={ text( 'eyebrow', placeholders.eyebrow || __( 'Eyebrow', 'floe' ), { allowedFormats: [] } ) }
			heading={ text( 'heading', placeholders.heading || __( 'Section heading', 'floe' ) ) }
			intro={ intro ? text( 'intro', placeholders.intro || __( 'Optional supporting line', 'floe' ) ) : null }
			action={
				action ? (
					<LinkButton
						...
						style={ actionStyle }
						arrow={ actionArrow }
						placeholder={ placeholders.action || __( 'Optional action', 'floe' ) }
					/>
				) : null
			}
			aside={ aside }
...
export function HeadingLevelControl( { value = 2, onChange, levels = [ 2, 3, 4 ] } ) {
	return (
		<SelectControl
			__next40pxDefaultSize
			__nextHasNoMarginBottom
			label={ __( 'Heading level', 'floe' ) }
			help={ __( 'Keep headings in order down the page. Only banners use H1.', 'floe' ) }
			value={ String( value ) }
			options={ levels.map( ( level ) => ( { label: `H${ level }`, value: String( level ) } ) ) }
...
export function SurfaceControl( { value, onChange, options } ) {
	...
	return (
		<SelectControl
			...
			value={ value }
			options={ options.map( ( option ) => ( { label: labels[ option ] || option, value: option } ) ) }
			onChange={ onChange }
		/>
	);
}
