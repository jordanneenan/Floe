...
function Edit( { attributes, setAttributes } ) {
			...
			<li { ...blockProps }>
				<MediaPlaceholder
					...
					labels={ {
						title: __( 'File', 'floe' ),
						instructions: __(
							'Upload or choose a PDF, Word, Excel or PowerPoint file.',
							'floe'
						),
					} }
					onSelect={ onSelect }
	...
	const extension = ( file?.source_url || '' )
		.split( '.' )
		.pop()
		.split( '?' )[ 0 ];
	const meta = [
		extension.toUpperCase(),
		formatSize( file?.media_details?.filesize ),
	]
		.filter( Boolean )
		.join( ' · ' );
			...
			<FileRow
				...
				title={
					<RichText
						...
						placeholder={
							file?.title?.rendered || __( 'File title', 'floe' )
						}
						allowedFormats={ [] }
						disableLineBreaks
					/>
