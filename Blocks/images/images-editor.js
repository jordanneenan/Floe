...
function Edit( { attributes, setAttributes, name } ) {
	...
	const blockProps = useFloeBlockProps( name, { className: `images--${ fit }` }, { surface: 'base' } );
	const innerBlocksProps = useInnerBlocksProps( { className: 'images__rows' }, { allowedBlocks: metadata.allowedBlocks, template: TEMPLATE } );
		...
		<>
			<InspectorControls>
				<PanelBody title={ __( 'Settings', 'floe' ) }>
					<SelectControl
						...
						label={ __( 'Fit', 'floe' ) }
						help={ __( 'Fill crops every image to the row’s shape. Natural keeps each file’s own proportions.', 'floe' ) }
						value={ fit }
						options={ [ { label: __( 'Fill', 'floe' ), value: 'fill' }, { label: __( 'Natural', 'floe' ), value: 'natural' } ] }
			...
			<section { ...blockProps }>
				<div className="images__inner">
					<EditableSectionHeader attributes={ attributes } setAttributes={ setAttributes } intro={ false } placeholders={ { eyebrow: __( 'Optional eyebrow', 'floe' ), heading: __( 'Optional heading', 'floe' ) } } />
