...
function Edit( { attributes, setAttributes, clientId, name } ) {
	...
	const blockProps = useFloeBlockProps(
		name,
		{
			className: `image-row--count-${ Math.min( 3, Math.max( 1, count ) ) }${ attributes.gap ? '' : ' image-row--no-gap' }`,
		},
		{ section: false }
	);
			...
			<InspectorControls>
				<PanelBody title={ __( 'Row', 'floe' ) }>
					<ToggleControl
						...
						label={ __( 'Gap between items', 'floe' ) }
						checked={ attributes.gap }
						onChange={ ( gap ) => setAttributes( { gap } ) }
					/>
					<p>
						{ __(
							'A row holds up to three items. The number of columns follows the number of items.',
							'floe'
						) }
					</p>
				</PanelBody>
