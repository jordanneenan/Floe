...
| --- | --- | --- |
| `blocks/<group>/<id>/` | Restructured | One folder per block directly under `Blocks/`; first block is `Blocks/Spacing/`. |
...
| Made front-end libraries and build pipeline | Not ported | Current build uses `@wordpress/scripts` and Sass for the one block. |
