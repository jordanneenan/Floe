...
export function PlayControl( { duration } ) {
	return (
		<span className="play-control">
			<span className="play-control__circle"><Icon name="play" /></span>
			<span className="play-control__text">
				<span className="play-control__label">{ __( 'Play video', 'floe' ) }</span>
				{ duration && <span className="play-control__duration">{ duration }</span> }
			</span>
		</span>
	);
}
