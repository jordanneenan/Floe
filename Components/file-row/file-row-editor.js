...
export const typeCode = ( extension = '' ) =>
	CODES[ extension.toLowerCase() ] || extension.toUpperCase().slice( 0, 4 );

export function FileRow( { extension = '', title, meta } ) {
	return (
		<span className="file-row">
			<span className="file-row__type" aria-hidden="true">
				{ typeCode( extension ) }
			</span>
			...
			<span className="file-row__action" aria-hidden="true">
				<Icon name="download" />
			</span>
		</span>
	);
