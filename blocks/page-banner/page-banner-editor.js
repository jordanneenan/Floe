...
function Edit( { attributes, setAttributes, name } ) {
	...
	const blockProps = useFloeBlockProps(
		name,
		{ className: showMedia ? 'has-media' : 'no-media' },
		{ surface }
	);
			...
			<InspectorControls>
				<PanelBody title={ __( 'Settings', 'floe' ) }>
					<SurfaceControl
						...
						options={ [ 'tint', 'subtle', 'base', 'inverse' ] }
					/>
					<ToggleControl
						__nextHasNoMarginBottom
						label={ __( 'Show breadcrumb', 'floe' ) }
						help={ __(
							'Built automatically from the page hierarchy.',
							'floe'
						) }
						checked={ showBreadcrumb }
						onChange={ ( next ) =>
							setAttributes( { showBreadcrumb: next } )
						}
					/>
					<ToggleControl
						__nextHasNoMarginBottom
						label={ __( 'Show media', 'floe' ) }
						checked={ showMedia }
						onChange={ ( next ) =>
							setAttributes( { showMedia: next } )
						}
					/>
				</PanelBody>
			</InspectorControls>
