			...
			<InspectorControls>
				<PanelBody title={ __( 'Links', 'floe' ) }>
					<p>
						{ items.length
							? __(
									'Using the links below. Remove them all to go back to automatic links.',
									'floe'
								)
							: __(
									'Automatic: every section on this page with an HTML anchor (Block settings → Advanced) gets a link, labelled with its eyebrow or heading.',
									'floe'
								) }
					</p>
							...
							{ shown.length ? (
								shown.map( ( item, index ) => (
									<li key={ item.anchor + index }>
										<span
											className={ `in-page-nav__link${ index === 0 ? ' is-active' : '' }` }
										>
								...
								<li className="floe-slot-hint">
									{ __(
										'Give sections an HTML anchor to list them here.',
										'floe'
									) }
								</li>
							) }
